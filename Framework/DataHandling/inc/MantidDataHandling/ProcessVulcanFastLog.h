#ifndef LOADEVENTPRENEXUS3_H_
#define LOADEVENTPRENEXUS3_H_
...
/** @class Mantid::DataHandling::ProcessVulcanFastLog
...
class DLLExport ProcessVulcanFastLog
    : public API::IFileLoader<Kernel::FileDescriptor> {
public:
  /// Constructor
  ProcessVulcanFastLog();
  /// Virtual destructor
  ~ProcessVulcanFastLog() override;
  /// Algorithm's name
  const std::string name() const override { return "ProcessVulcanFastLog"; }
  /// Algorithm's version
  ...
  /// Algorithm's aliases
  const std::string alias() const override { return "ProcessVulcanFastLog"; }
...
private:
  ...
  /// main vector to contain all the input
  std::vector<std::vector<Kernel::DateAndTime>> m_timeBlockVector;
  std::vector<std::vector<int64_t>> m_valueBlockVector;
